...
# API key. Set IDEALAB_KEY to the key file (default: the host's dashscope vault).
...
# workspace: idealab provider + vault key
...
# personal config: idealab + notes ssh url (sandbox-reachable container name)

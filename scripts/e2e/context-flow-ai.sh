...
# API key (ANTHROPIC_KEY, default: the host's example vault). Safe + self-
...
# workspace: anthropic provider + vault key + host config (display-clone, host url)
...
# write personal config with a given notes url (anthropic provider stays fixed)
set_notes_url() {

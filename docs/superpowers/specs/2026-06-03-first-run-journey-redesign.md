...
- `onboarding(ctx)`:仿 code.ts 三步,驱动 gate:
  1. `!personalRepoImported` → route `/settings/personal-repo`(用户填 fixture personal repo 地址 + git-crypt key)
  2. 无 AI key → form 要 AI key(值由 env 提供,action `vault-env`)
  3. ssh 探测:vault 的 key 能否 clone fixture 的 kn/notes → **不能**则 `info` 视图("把这个公钥加到平台 SSH Keys");**能**则 `{done:true}`
...
5. UI 配 **personal repo**:填 fixture 地址 + git-crypt key → loopat clone + unlock + `seedDefaults` → personal 配好。

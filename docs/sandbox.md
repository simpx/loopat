...
| 层 | 文件 | 谁写 | `src` 范围 | 例 |
|---|---|---|---|---|
| **operator** | `~/.dashscope/config.json` `mounts` | host shell 用户 | 任意 host 路径（`~/...`、`$HOME/...`、绝对 `/...`） | `$HOME/.cache`、`/etc/pki/ca-trust` |
| **admin** | `knowledge/.loopat/.claude/settings.json` + `knowledge/.loopat/profiles/<name>/.claude/settings.json` | 团队管理员（push knowledge git） | 无 mount 字段 | — |
...
**不 `--unshare-net`**，host 网络共享。Claude 调 `api.anthropic.com` / 百炼 / curl / git fetch / npm install / pip install 都直接走。

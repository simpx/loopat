 ...
 *   4. Write the user's personal config (idealab provider) and build a
 *      SELF-CONTAINED vault: a FRESH ssh keypair (generated here, not copied
 *      from any real vault) + IDEALAB_API_KEY taken from the env. Add a
...
async function globalSetup() {
  ...
  //     the loop's ${IDEALAB_API_KEY} resolves. Never read from disk, never committed.

 ...
 *
 * Setup: requires a real idealab/anthropic API key at
 * `vaults/<user>/.loopat/vaults/default/envs/IDEALAB_API_KEY` (or env var
 * IDEALAB_API_KEY on the test host).
 */
...
describe.skipIf(SKIP)("L4: real AI sees vault-derived env var", () => {
  test("AI can read $TEST_CANARY injected via vault → bwrap --setenv", async () => {
    ...
    const apiKey = process.env.IDEALAB_API_KEY ?? process.env.ANTHROPIC_API_KEY
    if (!apiKey) throw new Error("Need IDEALAB_API_KEY or ANTHROPIC_API_KEY env to run L4")

    const baseUrl = process.env.IDEALAB_API_KEY
    ...
    const it = query({
      ...
      options: {
        ...
        model: process.env.IDEALAB_API_KEY ? "claude-opus-4-7" : "claude-haiku-4-5-20251001",
        maxTurns: 3,

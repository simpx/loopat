  ...
  test("returns disk shape + refExists for each provider apiKey", async () => {
    await writeFile(personalLoopatConfigPath(USER), JSON.stringify({
      providers: {
        ...
        idealab:   { baseUrl: "https://idealab.example.com", model: "y", apiKey: "${IDEALAB_API_KEY}" },
    ...
    expect(j.refExists["providers.idealab.apiKey"]).toEqual({
      kind: "var", exists: false, varName: "IDEALAB_API_KEY",
    })

...
describe("loadPersonalConfig — ${VAR} resolution in provider.apiKey", () => {
  ...
  test("provider apiKey ${VAR} resolves from vault envs", async () => {
    await writeVaultEnv(USER, "default", "IDEALAB_API_KEY", "sk-secret-xyz")
    await writeConfig({
      providers: {
        default: "idealab",
        idealab: {
    ...
    expect(cfg.providers.idealab.apiKey).toBe("sk-secret-xyz")
    expect(cfg.vaultEnvs.IDEALAB_API_KEY).toBe("sk-secret-xyz")
  })

...
- **Given** `vaults/default/envs/ANTHROPIC_API_KEY=sk-xxx`, config.json apiKey=`${ANTHROPIC_API_KEY}`
- **When** loadPersonalConfig runs
- **Then**
  - L1: `cfg.providers.anthropic.apiKey === "sk-xxx"`
...
- **Given** config.json has apiKey=`${ANTHROPIC_API_KEY}` and vault has actual value "sk-secret"
- **When** GET /api/settings/personal/disk
- **Then**
  - L2: response body does NOT contain "sk-secret" anywhere
  - L2: disk.providers.anthropic.apiKey is the template string `"${ANTHROPIC_API_KEY}"`
